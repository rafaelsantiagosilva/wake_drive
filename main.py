...
def main():
    ...
    arduino = Serial("COM3", 9600)  # Substitua 'COM5' pela porta correta do seu Arduino

    closed_eye_start_time = None
    eye_closed_duration = (
        1  # Duration for which the eye should be closed to trigger the Arduino command
    )
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                dist = math.sqrt(
                    (cord1[0] - cord2[0]) ** 2 + (cord1[1] - cord2[1]) ** 2
                )
                dist2 = math.sqrt(
                    (cord4[0] - cord3[0]) ** 2 + (cord4[1] - cord3[1]) ** 2
                )
                ...
                if calibrating:
                    ...
                    if ratioMapped < blinkMap and piscando == False:
                        ...
                        closed_eye_start_time = (
                            time.time()
                        )  # Start timing when the eye is detected closed

                    if ratioMapped >= blinkMap + 1 and piscando == True:
                        piscando = False
                        closed_eye_start_time = (
                            None  # Reset timing when the eye is detected open
                        )
                    ...
                    if (
                        ...
                        blinkCount += 1
                        piscando = False
                        closed_eye_start_time = None  # Reset timing
                        # Envia um comando para o Arduino quando o olho estiver fechado por 1.5 segundos
                        arduino.write(b"1")
